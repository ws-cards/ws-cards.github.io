...
/* 匯出圖版面（CSS px；實際以 2x 繪製）
 *
 * 節奏：由上往下逐段壓縮 —— 頁首 340 / 走勢 396 / 數據條 96 / 頁尾 44。
 * 分隔線的重量跟著遞減：2px 金色實條 → 1px 金色細線 → 墨色塊。
 * 眼睛在上半部慢下來看價格，到下半部越掃越快。
 *
 * 度量：全圖只有一個邊界 GUTTER=32，三段共用同一條 736px 量尺。
 * 走勢圖的 Y 軸刻度收到圖內（壓在格線上方），折線才真的跟頁首、
 * 數據條齊寬 —— 原本刻度佔掉左邊 52px，折線比其他段窄一截。
 *
 * 間距用 4 的倍數：8 / 12 / 16 / 20 / 24 / 28 / 32 / 40 / 48。
 */
...
*
* 卡面的上下緣是對齊出來的，不是估的：上緣切齊卡名的字高頂線，
* 下緣切齊最後一行小字的基線。差 4px 的「差不多對齊」看起來是手滑，
* 真的對齊才看得出是排過的。
*
* 右欄只有兩個區塊、一個斷點：上面是「這是哪張卡」（卡名 + 屬性籤），
* 下面是「值多少」（卡號收錄 + 現價）。卡號／收錄／特徵是往上貼齊現價的，
* 不跟著卡名往下流 —— 這樣兩行卡名也不會擠到現價，
* 而且空白只會出現在那唯一一個斷點上，不會散成三段。
*/
function _drawStatsInkHead(ctx, W, L, theme, info) {
    ...
    var NAME_BASELINE = 60;
    var NAME_LEADING = 36;
    var FACTS_BASELINE = 152;
    var FACTS_LEADING = 20;
    var RULE_Y = 210;
    var LABEL_BASELINE = 230;
    var PRICE_BASELINE = 288;
    var SUB_BASELINE = 312;

    // ── 卡面：收藏品本身要先被看見 ──
    var boxX = L.ART_X, boxY = L.ART_Y, boxW = L.ART_W, boxH = L.ART_H;
    if (info.cardImage) {
    ...
    for (var n = 0; n < nameLines.length; n++) {
        ctx.fillText(nameLines[n], TX, NAME_BASELINE + n * NAME_LEADING);
    }
    var afterName = NAME_BASELINE + (nameLines.length - 1) * NAME_LEADING;

    var chipY = Math.min(afterName + 12, FACTS_BASELINE - 32);
    var chipX = TX;
    ...
    for (var f = 0; f < factRows.length; f++) {
        var rowY = FACTS_BASELINE + f * FACTS_LEADING;
        ctx.fillStyle = theme.inkGoldSoft;
        ...
        ctx.textAlign = 'left';
        ctx.fillText(_truncateText(ctx, factRows[f].value, colW - 56), TX + 56, rowY);
    }
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(TX, RULE_Y + 0.5);
    ctx.lineTo(RX, RULE_Y + 0.5);
    ctx.stroke();

    ctx.fillStyle = theme.inkGold;
    ctx.font = '700 10px "Noto Sans TC", sans-serif';
    ctx.textAlign = 'left';
    _drawTrackedText(ctx, '遊遊亭 現價 / JPY', TX, LABEL_BASELINE, 2.2);
    ...
    if (hasPrice) {
        ...
        ctx.fillText(_truncateText(ctx, info.currentPrice, colW * 0.62), TX, PRICE_BASELINE);
    } else {
        ctx.fillStyle = theme.inkGoldSoft;
        ctx.font = '700 26px "Noto Sans TC", sans-serif';
        ctx.fillText('目前沒有價格紀錄', TX, PRICE_BASELINE - 10);
    }
    ...
    if (hasChange) {
        ...
        ctx.font = '700 30px "Noto Sans TC", sans-serif';
        ctx.fillText(info.marketChangePercent, RX, PRICE_BASELINE - 6);
        if (direction !== 0) {
            var changeWidth = ctx.measureText(info.marketChangePercent).width;
            _drawStatsTriangle(ctx, RX - changeWidth - 15, PRICE_BASELINE - 16, 12, direction, directionColor);
        }

        ctx.fillStyle = theme.inkGoldSoft;
        ctx.font = '500 11px "Noto Sans TC", sans-serif';
        var amount = (info.marketChangeAmount && info.marketChangeAmount !== '--') ? info.marketChangeAmount + '　' : '';
        _drawTrackedText(ctx, amount + '近 30 日', RX, SUB_BASELINE, 0.6, 'right');
        ctx.textAlign = 'left';
    }
}
...
* 期間最高／最低標在線上，數據條講的是近 30 日，兩個窗口都寫清楚。
*
* 圖例併進標題列 —— 它是關於這張圖的說明，不是圖裡的資料，
* 放在圖內只會跟最高／最低的標籤搶同一個角。
*/
function _drawStatsTrend(ctx, W, L, theme, info) {
    ...
    var titleBaseline = L.TITLE_BASELINE;

    // 段落標題
    ctx.fillStyle = theme.paperGold;
    ctx.fillRect(G, titleBaseline - 16, 4, 18);
    ctx.fillStyle = theme.paperText;
    ctx.font = '700 16px "Noto Sans TC", sans-serif';
    ctx.textAlign = 'left';
    ctx.fillText('價格與庫存走勢', G + 16, titleBaseline);
    ...
    var hasStock = points.some(function(p) { return p.stock !== null; });

    // 標題列右側：期間 + 圖例，由右往左排
    var months = (series && series.months) ? series.months : 12;
    var metaX = W - G;
    ctx.font = '500 11px "Noto Sans TC", sans-serif';
    ctx.fillStyle = theme.paperMuted;
    metaX -= _drawTrackedText(ctx, '近 ' + months + ' 個月', metaX, titleBaseline, 0.6, 'right');
    ...
    if (pricePoints.length >= 2) {
        ...
        if (hasStock) {
            metaX -= 24;
            ctx.fillStyle = theme.paperMuted;
            metaX -= _drawTrackedText(ctx, '庫存', metaX, titleBaseline, 1.2, 'right');
            metaX -= 7;
            // 底柱在圖裡是刻意壓低的配角，但圖例裡要找得到，所以補一道細框
            ctx.fillStyle = theme.column;
            ctx.fillRect(metaX - 9, titleBaseline - 9, 9, 9);
            ctx.strokeStyle = theme.paperHairline;
            ctx.lineWidth = 1;
            ctx.strokeRect(metaX - 8.5, titleBaseline - 8.5, 8, 8);
            metaX -= 9;
        }
        metaX -= 20;
        ctx.fillStyle = theme.paperMuted;
        metaX -= _drawTrackedText(ctx, '價格', metaX, titleBaseline, 1.2, 'right');
        metaX -= 7;
        ctx.strokeStyle = theme.line;
        ctx.lineWidth = 2.5;
        ctx.beginPath();
        ctx.moveTo(metaX - 15, titleBaseline - 4);
        ctx.lineTo(metaX, titleBaseline - 4);
        ctx.stroke();
    }
    ctx.textAlign = 'left';

    if (pricePoints.length < 2) {
        // 空狀態不要撐滿整個繪圖區：268px 的虛線框看起來像掉了東西。
        // 收成一塊置中的淺框，空白就成了留白，而不是破洞。
        var emptyH = 136;
        var emptyY = plotY + Math.round((plotH - emptyH) / 2);
        ctx.strokeStyle = theme.paperHairline;
        ctx.lineWidth = 1;
        ctx.setLineDash([4, 4]);
        ctx.beginPath();
        roundRectPath(ctx, plotX + 0.5, emptyY + 0.5, plotW - 1, emptyH - 1, 6);
        ctx.stroke();
        ctx.setLineDash([]);
        ctx.fillStyle = theme.paperMuted;
        ctx.font = '500 13px "Noto Sans TC", sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText('這張卡目前的價格紀錄不足，畫不出走勢。', plotX + plotW / 2, emptyY + emptyH / 2 + 5);
        ctx.textAlign = 'left';
        return;
    }
    ...
    for (var tick = axis.lo; tick <= axis.hi + axis.step * 0.01; tick += axis.step) {
        ...
        ctx.textAlign = 'left';
        ctx.fillText(Math.round(tick).toLocaleString('en-US'), plotX, ty - 6);
    }
    ctx.textAlign = 'left';
    ...
    function markExtreme(index, label, above) {
        ...
        labelY = Math.max(plotY + 14, Math.min(plotBottom - 6, labelY));
    ...
    // 「今日」掛在時間軸上，右端的月份讓位給它。
    var axisBaseline = L.AXIS_BASELINE;
    ctx.fillStyle = theme.paperGold;
    ctx.font = '700 10px "Noto Sans TC", sans-serif';
    var todayAnchor = Math.min(lastX + 12, plotX + plotW);
    var todayLabelW = _drawTrackedText(ctx, '今日', todayAnchor, axisBaseline, 1.8, 'right');
    var todayLabelLeft = todayAnchor - todayLabelW;
...
* 四格的文字左緣間距相等（184px 一格），分隔線退到每格左緣前 14px ——
* 第一格靠版面邊界、其他格靠分隔線的話，四個數字的左緣就會排不齊。
...
function _drawStatsMarketStrip(ctx, W, L, theme, info) {
    ...
    for (var i = 0; i < cells.length; i++) {
        ...
        if (i > 0) {
            ...
            ctx.lineWidth = 1;
            ctx.beginPath();
            ctx.moveTo(Math.round(textX - 14) + 0.5, top + 14);
            ctx.lineTo(Math.round(textX - 14) + 0.5, bottom - 14);
            ctx.stroke();
        }
        ...
        ctx.fillStyle = theme.paperGold;
        ctx.font = '700 10px "Noto Sans TC", sans-serif';
        _drawTrackedText(ctx, cells[i].label, textX, top + 26, 1.8);

        ctx.fillStyle = hasValue ? cells[i].color : theme.paperSubtle;
        ctx.font = '700 24px "Noto Sans TC", sans-serif';
        ctx.textAlign = 'left';
        ctx.fillText(_truncateText(ctx, cells[i].value, cellW - 22), textX, top + 58);

        if (cells[i].note) {
            ctx.fillStyle = theme.paperSubtle;
            ctx.font = '500 10px "Noto Sans TC", sans-serif';
            _drawTrackedText(ctx, cells[i].note, textX, top + 78, 0.8);
        }
    }
}
