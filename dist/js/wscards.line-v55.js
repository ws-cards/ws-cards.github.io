...
var PSA_GRADES = ['10', '9', '8以下'];
var BGS_GRADES = ['黑10', '金10', '9.5', '9以下'];
var ARS_GRADES = ['10+', '10', '9', '8以下'];
...
function getGradesForCompany(company) {
    switch (company) {
        ...
        case 'ARS': return ARS_GRADES;
        default: return PSA_GRADES;
    }
}
...
function getBarClass(grade) {
    ...
    if (grade.indexOf('以下') > -1) return 'grade-low';
...
function renderGradingData() {
    ...
    var grades = getGradesForCompany(currentCompany);
    ...
    if (companyData._updateDate) {
        html += '<div style="font-size:0.75rem; color:var(--text-secondary); text-align:right; margin-bottom:0.25rem;"><i class="fas fa-clock mr-1"></i>資料更新日期: ' + companyData._updateDate + '</div>';
    }

    html += '<div class="grading-total-row">';
    html += '<span><i class="fas fa-layer-group mr-2"></i>總鑑定數量</span>';
    ...
    grades.forEach(function(grade) {
        ...
        var isBlack10 = (grade === '黑10');
        var barClass = isBlack10 ? 'grade-black10' : getBarClass(grade);
        gradeStats.push({ grade: grade, count: count, pct: pct, barHeight: barHeight, isTen: isTen, isBlack10: isBlack10, barClass: barClass });
    });

    html += '<div class="grading-chart">';
    gradeStats.forEach(function(s) {
        var colClass = 'grading-col';
        if (s.isBlack10) colClass += ' is-black10';
        else if (s.isTen) colClass += ' is-ten';
    ...
    gradeStats.forEach(function(s) {
        var thCls = s.isBlack10 ? 'th-black10' : (s.isTen ? 'th-ten' : '');
        html += '<th class="' + thCls + '">' + s.grade + '</th>';
    });
    html += '</tr></thead>';
    html += '<tbody>';
    ...
    gradeStats.forEach(function(s) {
        var tdCls = s.isBlack10 ? 'td-black10' : (s.isTen ? 'td-ten' : '');
        html += '<td class="' + tdCls + '">' + s.count + '</td>';
    });
    html += '</tr>';
    ...
    if (tenRateContainer) {
        var tenRate = totalCount > 0 ? ((tenCount / totalCount) * 100).toFixed(1) : '0.0';
        document.getElementById('tenRateValue').textContent = tenRate + '%';
        tenRateContainer.style.display = 'flex';
    }
}
