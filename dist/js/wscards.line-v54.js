	...
	*/
	...
	if (cardNumber && cardNumber !== '000/000-000') {
		...
		if (typeof GradingModule !== 'undefined') {
			GradingModule.loadGradingData(cardTilteReplaceSpare, cardNumber);
		}
	}
...
var GradingModule = (function() {
     ...
     * @param {string} cardNumber - 卡號（內部格式或顯示格式皆可）
     */
    function loadGradingData(titleCode, cardNumber) {
        if (!titleCode || !cardNumber || cardNumber === '000/000-000') {
            resetUI();
            return;
        }

        // 同時取得顯示用卡號（用於模擬資料查找）
        var displayCardNumber = cardNumber;
        var cardNumberSelect = document.getElementById('cardNumber');
        if (cardNumberSelect && cardNumberSelect.selectedIndex >= 0) {
            displayCardNumber = cardNumberSelect.options[cardNumberSelect.selectedIndex].text || cardNumber;
        }

        var url = requestURLGradingBase + titleCode + '.json';
        console.log('載入鑑定資料:', url, '卡號:', cardNumber, '顯示卡號:', displayCardNumber);
        ...
        fetch(url)
            ...
            .then(function(data) {
                if (data && (data[cardNumber] || data[displayCardNumber])) {
                    gradingData = data[cardNumber] || data[displayCardNumber];
            ...
            .catch(function(error) {
                ...
                var mock = findMockData(cardNumber) || findMockData(displayCardNumber);
                if (mock) {
                    gradingData = mock;
                    console.log('使用模擬鑑定資料:', displayCardNumber);
                } else {
                    gradingData = null;
                    console.log('此卡片無鑑定資料（含模擬）:', displayCardNumber);
                }
                updateButtonStates();
                renderGradingData();
            });
