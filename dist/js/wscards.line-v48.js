...
function getCardData(jsonObj,internalCardNumber,cardNum) {
	console.log("進入繪圖區:"+cardNum);
				...
				// 更新價格摘要統計
				updatePriceSummary(cardData, cardPriceUpDate);
				
				// 更新歷史紀錄中的價格
				if (cardData && cardData.length > 0) {
				    var latestPrice = cardData[cardData.length - 1];
				    var history = getSearchHistory();
				    if (history.length > 0 && latestPrice) {
				        history[0].currentPrice = latestPrice;
...
function updateCardInfo(cardData) {
    ...
    addSearchHistoryRecord({
        ...
        currentPrice: '-',  // 價格會在 getCardData 中更新
        color: colorHistoryMap[cardData.cardcolor] || '#667eea'
    });
}
 ...
 * @param {Array} dateData - 日期陣列
 */
function updatePriceSummary(priceData, dateData) {
    if (!priceData || priceData.length === 0) return;
    
    // 過濾掉 null / 0 的無效值
    var validPrices = priceData.filter(function(p) { return p !== null && p !== undefined && p > 0; });
    ...
    var changePercent = firstPrice > 0 ? ((currentPrice - firstPrice) / firstPrice * 100).toFixed(1) : 0;
    
    // 更新 DOM
    document.getElementById('statCurrentPrice').textContent = '¥' + currentPrice.toLocaleString();
    document.getElementById('statHighPrice').textContent = '¥' + highPrice.toLocaleString();
    document.getElementById('statLowPrice').textContent = '¥' + lowPrice.toLocaleString();
    
    var changeEl = document.getElementById('statChangePercent');
    var changeIconEl = document.getElementById('statChangeIcon');
    
    if (changePercent > 0) {
        changeEl.textContent = '+' + changePercent + '%';
        changeEl.className = 'stat-value price-up';
        changeIconEl.style.background = 'linear-gradient(135deg, #e53e3e, #fc8181)';
    } else if (changePercent < 0) {
        changeEl.textContent = changePercent + '%';
        changeEl.className = 'stat-value price-down';
        changeIconEl.style.background = 'linear-gradient(135deg, #38a169, #68d391)';
    } else {
        changeEl.textContent = '0%';
        changeEl.className = 'stat-value';
        changeIconEl.style.background = 'linear-gradient(135deg, #a0aec0, #cbd5e0)';
    }
    
    // 更新時間
    if (dateData && dateData.length > 0) {
        document.getElementById('statUpdateTime').textContent = '資料區間：' + dateData[0] + ' ~ ' + dateData[dateData.length - 1];
    }
    
    // 顯示統計卡片
    document.getElementById('priceSummaryCard').style.display = 'block';
}
...
var HISTORY_KEY = 'wsCards_searchHistory';
var HISTORY_MAX = 10;
...
/**
 * 新增一筆搜尋紀錄
 * @param {object} record - { cardno, cardname, currentPrice, color }
 */
function addSearchHistoryRecord(record) {
    if (!record || !record.cardno) return;
    
    var history = getSearchHistory();
    
    // 移除重複項
    history = history.filter(function(h) { return h.cardno !== record.cardno; });
...
function renderSearchHistory() {
    ...
    if (history.length === 0) {
        container.innerHTML = '<div class="history-empty">' +
            ...
            '<p style="color:var(--text-secondary); font-size:0.9rem;">尚無查看紀錄</p>' +
            '</div>';
        return;
    }
    ...
    history.forEach(function(item) {
        html += '<div class="history-item" onclick="onHistoryItemClick(\'' + item.cardno.replace(/'/g, "\\'") + '\')">' +
            '<div class="history-item-icon" style="background:' + item.color + ';">' +
                '<i class="fas fa-tag"></i>' +
            '</div>' +
            '<div class="history-item-info">' +
                '<div class="history-item-cardno">' + item.cardno + '</div>' +
                '<div class="history-item-name">' + item.cardname + '</div>' +
            '</div>' +
            '<div class="history-item-price">' + (item.currentPrice !== '-' ? '¥' + Number(item.currentPrice).toLocaleString() : '') + '</div>' +
        '</div>';
    });
 ...
 * @param {string} cardno
 */
function onHistoryItemClick(cardno) {
    var inputEl = document.getElementById('xxxx');
    if (inputEl) {
        inputEl.value = cardno;
        inputEl.dispatchEvent(new Event('input', { bubbles: true }));
    }
    // 延遲後直接搜尋
    setTimeout(function() {
        if (isCardNumberFormat(cardno)) {
            searchByCardNumber(cardno);
        }
    }, 200);
}
            ...
            if (typeof Swal !== 'undefined') {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        clearSearchHistory();
                        Swal.fire({ icon: 'success', title: '已清除', timer: 1000, showConfirmButton: false });
                    }
                });
            } else {
                if (confirm('確認清除所有查看紀錄？')) {
                    clearSearchHistory();
                }
            }
        });
