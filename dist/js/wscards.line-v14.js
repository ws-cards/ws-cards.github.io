...
function isCardNumberFormat(input) {
  ...
  // 後綴卡號格式: SET-NUMBER (例如: W01-001, W54-070SSP, S111-033)
  // 修正：支援更靈活的數字字母組合
  var suffixCardPattern = /^[A-Z]+[0-9]*-[0-9]+[A-Z]*$/i;
...
function parseCardNumber(cardNumber) {
  ...
  // 格式2: 後綴卡號格式 SET-NUMBER (例如: W01-001, S111-033)
  // 修正：支援更靈活的數字字母組合
  var suffixMatch = cardNumber.match(/^([A-Z]+[0-9]*)-([0-9]+[A-Z]*)$/i);
  if (suffixMatch) {
    var result = {
      prefix: null,                        // 沒有前綴
      series: null,                        // 沒有系列
      fullNumber: null,                    // 沒有完整卡號
      suffix: cardNumber                   // 例如: W01-001, S111-033
    };
 ...
 * 快速測試 S111-033 格式
 */
function testS111033() {
  console.log('=== S111-033 格式測試 ===');
  ...
  // 1. 格式檢查
  var isValid = isCardNumberFormat(input);
  console.log('1. 格式檢查:', isValid ? '✓ 有效' : '✗ 無效');
  ...
  if (isValid) {
    ...
    if (parsed) {
      ...
      console.log('3. ✓ S111-033 格式現在應該可以正常搜尋了！');
    } else {
      console.log('2. ✗ 解析失敗');
    }
  }
  
  console.log('\n💡 在控制台中輸入 testS111033() 來快速測試 S111-033 格式');
}
