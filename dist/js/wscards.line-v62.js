...
function renderCardNameSearchResults(results) {
    ...
    results.forEach(function(item) {
        var cardNo = item[0];
        var cardName = item[1];
        html += '<div class="card-name-search-item" style="padding: 10px; border-bottom: 1px solid #eee; cursor: pointer; display: flex; justify-content: space-between;" ' + 
                'onclick="handleCardNameResultClick(\'' + cardNo + '\')">';
        html += '<span style="font-weight: bold; font-size: 0.9em; flex: 1; margin-right: 10px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="'+cardName+'">' + cardName + '</span>';
        html += '<span style="color: #667eea; font-size: 0.8em; flex-shrink: 0;">' + cardNo + '</span>';
        html += '</div>';
    });
    ...
    // 加一點 hover 效果 (可透過 CSS 或 JS，這邊用 JS)
    var items = searchDiv.querySelectorAll('.card-name-search-item');
