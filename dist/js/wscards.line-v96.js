...
function getStatsExportTheme() {
    ...
    return dark ? {
        ...
        textSubtle: '#8f9388',
        price: '#9fc5ff',
        priceFill: 'rgba(159, 197, 255, 0.12)',
        stock: '#e2b96c',
        stockFill: 'rgba(226, 185, 108, 0.58)',
        positive: '#72c79b',
        negative: '#ff8b82',
        grid: 'rgba(242, 240, 232, 0.12)',
        tag: '#d9e7ff',
        tagSurface: '#293b54',
        accent: '#d9b96d'
    } : {
        ...
        textSubtle: '#7b8490',
        price: '#183d6d',
        priceFill: 'rgba(24, 61, 109, 0.10)',
        stock: '#b77718',
        stockFill: 'rgba(214, 147, 35, 0.62)',
        positive: '#237a52',
        negative: '#bf3f43',
        grid: 'rgba(23, 34, 53, 0.12)',
        tag: '#183d6d',
        tagSurface: '#e7edf5',
        accent: '#9b681a'
    };
...
function _composeStatsCanvas(info) {
    ...
    if (info.cardImage) {
        try {
            ...
            ctx.shadowColor = theme.isDark ? 'rgba(0,0,0,0.42)' : 'rgba(38,31,18,0.18)';
            ctx.shadowBlur = 14;
            ctx.shadowOffsetX = 2;
            ctx.shadowOffsetY = 4;
            ctx.save();
    ...
    var badgeX = TX;
    if (info.cardRare) {
        badgeX += drawBadge(info.cardRare, badgeX, TY, theme.tagSurface, theme.tag);
    }
    if (info.cardColor) {
    ...
    if (info.cardLevel) {
        badgeX += drawBadge('Lv.' + info.cardLevel, badgeX, TY, theme.isDark ? '#284436' : '#e4f3ea', C_DOWN);
    }
    if (info.cardPower) {
        badgeX += drawBadge('P.' + info.cardPower, badgeX, TY, theme.isDark ? '#4f302e' : '#fbe8e6', C_UP);
    }
    TY += 36;
...
function _drawStatsFooter(ctx, imgW, imgH, footerH, fy, pad, info, theme) {
    ...
    // 左：品牌識別。匯出圖是戰報，不模擬一個不存在的搜尋欄。
    ctx.textAlign = 'left';
    ctx.fillStyle = theme.accent;
    ctx.font = 'bold 12px "Noto Sans TC", sans-serif';
    ctx.fillText('WS-Cards', pad, textY);

    ctx.fillStyle = theme.textSubtle;
    ctx.font = '12px "Noto Sans TC", sans-serif';
    ctx.fillText('收藏戰報', pad + 72, textY);
