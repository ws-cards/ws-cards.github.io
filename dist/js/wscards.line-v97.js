...
function getStatsExportTheme() {
    ...
    return dark ? {
        ...
        textSubtle: '#8f9388',
        price: '#f2c96d',
        priceFill: 'rgba(242, 201, 109, 0.12)',
        stock: '#d8c8a0',
        stockFill: 'rgba(216, 200, 160, 0.56)',
        positive: '#72c79b',
        negative: '#ff8b82',
        grid: 'rgba(255, 241, 202, 0.15)',
        tag: '#ffe1a0',
        tagSurface: '#49391d',
        chartBackground: '#261f18',
        chartSurface: '#30271d',
        chartBorder: '#6c5832',
        accent: '#d9b96d'
    } : {
        ...
        textSubtle: '#7b8490',
        price: '#c2871f',
        priceFill: 'rgba(194, 135, 31, 0.12)',
        stock: '#7c6032',
        stockFill: 'rgba(196, 157, 82, 0.62)',
        positive: '#237a52',
        negative: '#bf3f43',
        grid: 'rgba(255, 234, 184, 0.16)',
        tag: '#714f19',
        tagSurface: '#f8e6bd',
        chartBackground: '#30271d',
        chartSurface: '#3a2e20',
        chartBorder: '#8d7040',
        accent: '#9b681a'
    };
...
* @returns {string}
*/
function _createStatsComboChartBase64(theme) {
    if (!_rawPriceLabels || _rawPriceLabels.length === 0) return '';
    theme = theme || getStatsExportTheme();
    ...
    var yAxes = [{
        ...
        ticks: {
            fontColor: theme.price,
            fontSize: 11,
            callback: function(value) {
                // 與走勢圖同一個修正：門檻 1000 會把 1300/1400 都印成「1k」。
                // 這張圖是會被下載、被轉貼出去的，刻度更不能說謊。
                if (value >= 10000) return (value / 1000).toFixed(0) + 'k';
                return value.toLocaleString('en-US', { maximumFractionDigits: 0 });
            }
        }
...
if (!statsWindow) {
    Swal.fire({ icon: 'warning', title: '無法開啟新分頁', text: '請允許此網站開啟新分頁後再試一次。', confirmButtonText: '知道了' });
    return;
}

// 卡圖非同步讀取期間仍要維持同一個主題，避免圖表和戰報框架來自不同模式。
...
var chartBase64 = '';
try {
    chartBase64 = _createStatsComboChartBase64(exportTheme);
...
function _composeStatsCanvas(info) {
    ...
    // 卡面、身分、市價、趨勢是一張市場海報的四個段落，而不是四張分離的 UI 卡。
    var HEADER_H = 286;
    var CHART_PANEL_H = 354;
    var FOOTER_H = 28;
    var SECTION_GAP = 14;
    ...
    var C_BG        = theme.background;
    var C_CARD      = theme.surface;
    var C_BORDER    = theme.border;
    var C_TEXT1     = theme.text;
    var C_TEXT2     = theme.textMuted;
    var C_TEXT3     = theme.textSubtle;
    var C_ACCENT    = theme.price;
    var C_UP        = theme.negative;
    var C_DOWN      = theme.positive;
    var C_NEUTRAL   = theme.textSubtle;

    ctx.fillStyle = C_BG;
    ctx.fillRect(0, 0, IMG_WIDTH, IMG_HEIGHT);

    function drawCard(x, y, w, h, radius) {
        ctx.shadowColor = theme.isDark ? 'rgba(0,0,0,0.26)' : 'rgba(38,31,18,0.09)';
        ctx.shadowBlur = 9;
        ctx.shadowOffsetY = 2;
    ...
    function drawBadge(text, x, y, bgColor, textColor) {
        ...
        var bw = tw + 18;
        var bh = 26;
        ctx.fillStyle = bgColor || theme.tagSurface;
        roundRect(ctx, x, y, bw, bh, 6);
        ctx.fill();
        ctx.fillStyle = textColor || theme.tag;
        ctx.textAlign = 'left';
    ...
    if (info.cardImage) {
        try {
            ...
            ctx.shadowColor = theme.isDark ? 'rgba(0,0,0,0.46)' : 'rgba(99,68,18,0.24)';
            ctx.shadowBlur = 16;
            ctx.shadowOffsetX = 2;
            ctx.shadowOffsetY = 4;
            ctx.save();
    ...
    var TX = hx + 224;
    var MARKET_W = 178;
    var MARKET_X = hx + hw - MARKET_W - 18;
    var TW = MARKET_X - TX - 22;
    var TY = hy + 24;

    ctx.fillStyle = theme.accent;
    ctx.font = 'bold 10px "Noto Sans TC", sans-serif';
    ctx.textAlign = 'left';
    ctx.fillText('WS-CARDS  /  MARKET REPORT', TX, TY);

    ctx.fillStyle = C_TEXT1;
    ctx.font = 'bold 24px "Noto Sans TC", sans-serif';
    ctx.fillText(_truncateText(ctx, info.cardName || info.cardNo || '卡片資料', TW), TX, TY + 34);

    var badgeX = TX;
    var badgeY = TY + 49;
    if (info.cardRare) {
        badgeX += drawBadge(info.cardRare, badgeX, badgeY, theme.tagSurface, theme.tag);
    }
    if (info.cardColor) {
        var colorBg = theme.isDark
            ? { '藍':'#263d57', '赤':'#4f302e', '紅':'#4f302e', '黃':'#4b4227', '黄':'#4b4227', '綠':'#284436', '緑':'#284436' }[info.cardColor]
            : { '藍':'#e4eef9', '赤':'#fbe8e6', '紅':'#fbe8e6', '黃':'#fbf1d8', '黄':'#fbf1d8', '綠':'#e4f3ea', '緑':'#e4f3ea' }[info.cardColor];
        var colorFg = { '藍':'#3182ce', '赤':'#d4534e', '紅':'#d4534e', '黃':'#bd8114', '黄':'#bd8114', '綠':'#329564', '緑':'#329564' }[info.cardColor] || C_TEXT2;
        badgeX += drawBadge(info.cardColor, badgeX, badgeY, colorBg, colorFg);
    }
    if (info.cardLevel) {
        badgeX += drawBadge('Lv.' + info.cardLevel, badgeX, badgeY, theme.isDark ? '#284436' : '#e4f3ea', C_DOWN);
    }
    if (info.cardPower) {
        badgeX += drawBadge('P.' + info.cardPower, badgeX, badgeY, theme.isDark ? '#4f302e' : '#fbe8e6', C_UP);
    }
    ...
    var identityRows = [
        ...
        { label: '發售', value: info.cardFirstRelease || '--' }
    ];
    var identityY = TY + 94;
    for (var identityIndex = 0; identityIndex < identityRows.length; identityIndex++) {
        var identity = identityRows[identityIndex];
        var rowY = identityY + identityIndex * 30;
        ctx.fillStyle = identityIndex % 2 ? theme.surfaceMuted : theme.surfaceRaised;
        roundRect(ctx, TX, rowY, TW, 25, 4);
        ctx.fill();
        ctx.fillStyle = C_TEXT3;
        ctx.font = '10px "Noto Sans TC", sans-serif';
        ctx.textAlign = 'left';
        ctx.fillText(identity.label, TX + 9, rowY + 16);
        ctx.fillStyle = C_TEXT1;
        ctx.font = '600 10px "Noto Sans TC", sans-serif';
        ctx.textAlign = 'right';
        ctx.fillText(_truncateText(ctx, identity.value, TW - 50), TX + TW - 9, rowY + 16);
    }

    // ── 3. 核心市價：高／現／低以一個緊湊區間講完 ──
    ctx.fillStyle = theme.chartSurface;
    ctx.strokeStyle = theme.chartBorder;
    ctx.lineWidth = 1;
    roundRect(ctx, MARKET_X, hy + 18, MARKET_W, 246, 8);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = theme.accent;
    ctx.fillRect(MARKET_X + 1, hy + 19, MARKET_W - 2, 5);

    ctx.fillStyle = '#fff4d8';
    ctx.font = 'bold 10px "Noto Sans TC", sans-serif';
    ctx.textAlign = 'left';
    ctx.fillText('MARKET VALUE', MARKET_X + 14, hy + 45);
    ctx.fillStyle = '#ffffff';
    ctx.font = 'bold 26px "Noto Sans TC", sans-serif';
    ctx.fillText(_truncateText(ctx, info.currentPrice || '--', MARKET_W - 28), MARKET_X + 14, hy + 78);
    ctx.fillStyle = '#d8c8a0';
    ctx.font = '10px "Noto Sans TC", sans-serif';
    ctx.fillText('遊遊亭最新售價 / JPY', MARKET_X + 14, hy + 96);

    var isNegative = String(info.marketChangePercent || '').indexOf('-') === 0;
    var changeColor = info.marketChangePercent && info.marketChangePercent !== '--'
        ? (isNegative ? '#ff9b8e' : '#8fe0ad') : '#d8c8a0';
    ctx.fillStyle = changeColor;
    ctx.font = 'bold 18px "Noto Sans TC", sans-serif';
    ctx.fillText(_truncateText(ctx, info.marketChangePercent || '--', MARKET_W - 28), MARKET_X + 14, hy + 125);
    ctx.font = 'bold 11px "Noto Sans TC", sans-serif';
    ctx.fillText(_truncateText(ctx, info.marketChangeAmount || '--', MARKET_W - 28), MARKET_X + 14, hy + 142);
    ctx.fillStyle = '#d8c8a0';
    ctx.font = '10px "Noto Sans TC", sans-serif';
    ctx.fillText('近 30 日變化', MARKET_X + 14, hy + 158);

    var rangeRows = [
        { label: '高', value: info.highPrice || '--', note: info.highDate || '--', color: '#ff9b8e' },
        { label: '低', value: info.lowPrice || '--', note: info.lowDate || '--', color: '#8fe0ad' },
        { label: '庫存', value: (info.currentStock || '--') + ' 件', note: info.dayChangeStock || '--', color: '#f2c96d' }
    ];
    for (var rangeIndex = 0; rangeIndex < rangeRows.length; rangeIndex++) {
        var range = rangeRows[rangeIndex];
        var rangeY = hy + 177 + rangeIndex * 27;
        ctx.strokeStyle = 'rgba(255, 241, 202, 0.18)';
        ctx.lineWidth = 0.5;
        ctx.beginPath();
        ctx.moveTo(MARKET_X + 14, rangeY - 9);
        ctx.lineTo(MARKET_X + MARKET_W - 14, rangeY - 9);
        ctx.stroke();
        ctx.fillStyle = range.color;
        ctx.font = 'bold 10px "Noto Sans TC", sans-serif';
        ctx.textAlign = 'left';
        ctx.fillText(range.label, MARKET_X + 14, rangeY + 4);
        ctx.fillStyle = '#ffffff';
        ctx.font = 'bold 12px "Noto Sans TC", sans-serif';
        ctx.fillText(_truncateText(ctx, range.value, 95), MARKET_X + 40, rangeY + 4);
        ctx.fillStyle = '#d8c8a0';
        ctx.font = '9px "Noto Sans TC", sans-serif';
        ctx.textAlign = 'right';
        ctx.fillText(_truncateText(ctx, range.note, 35), MARKET_X + MARKET_W - 14, rangeY + 4);
    }
    ...
    ctx.strokeStyle = theme.chartBorder;
    ctx.lineWidth = 1;
    roundRect(ctx, PAD, cy0, cpW, cpH, 9);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = theme.accent;
    ctx.fillRect(PAD + 18, cy0 + 18, 4, 24);
    var titleText = '遊遊亭  價格與庫存走勢';
    ctx.font = 'bold 14px "Noto Sans TC", sans-serif';
    ctx.fillStyle = '#fff4d8';
    ctx.textAlign = 'left';
    ctx.fillText(titleText, PAD + 31, cy0 + 35);
    ctx.fillStyle = '#d8c8a0';
    ctx.font = '10px "Noto Sans TC", sans-serif';
    ctx.textAlign = 'right';
    ctx.fillText('價格：日圓  /  庫存：件數', PAD + cpW - 20, cy0 + 34);

    var SIDEBAR_W = 136;
    var CH_X = PAD + 20;
    var CH_Y = cy0 + 58;
    var CH_W = cpW - SIDEBAR_W - 50;
    var CH_H = cpH - 78;
    var SUMMARY_X = CH_X + CH_W + 20;

    ctx.strokeStyle = 'rgba(255, 241, 202, 0.22)';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(SUMMARY_X - 10, cy0 + 58);
    ctx.lineTo(SUMMARY_X - 10, cy0 + cpH - 20);
    ctx.stroke();

    var compareRows = [
        { label: '30日最高', value: info.highPrice || '--', color: '#ff9b8e' },
        { label: '目前市價', value: info.currentPrice || '--', color: '#f2c96d' },
        { label: '30日最低', value: info.lowPrice || '--', color: '#8fe0ad' }
    ];
    for (var compareIndex = 0; compareIndex < compareRows.length; compareIndex++) {
        var compare = compareRows[compareIndex];
        var compareY = cy0 + 88 + compareIndex * 74;
        ctx.fillStyle = compare.color;
        ctx.font = 'bold 10px "Noto Sans TC", sans-serif';
        ctx.textAlign = 'left';
        ctx.fillText(compare.label, SUMMARY_X, compareY);
        ctx.fillStyle = '#ffffff';
        ctx.font = 'bold 17px "Noto Sans TC", sans-serif';
        ctx.fillText(_truncateText(ctx, compare.value, SIDEBAR_W - 4), SUMMARY_X, compareY + 25);
        if (compareIndex < compareRows.length - 1) {
            ctx.strokeStyle = 'rgba(255, 241, 202, 0.16)';
            ctx.lineWidth = 0.5;
            ctx.beginPath();
            ctx.moveTo(SUMMARY_X, compareY + 42);
            ctx.lineTo(SUMMARY_X + SIDEBAR_W - 8, compareY + 42);
            ctx.stroke();
        }
    }
    ...
    if (info.chartBase64) {
        ...
        chartImg.onload = function() {
            ...
            _drawStatsFooter(ctx, IMG_WIDTH, IMG_HEIGHT, FOOTER_H, cy0 + cpH + SECTION_GAP, PAD, info, theme);
...
* 繪製底部資訊列（浮水印 + 日期 + 來源）
*/
function _drawStatsFooter(ctx, imgW, imgH, footerH, fy, pad, info, theme) {
    theme = theme || getStatsExportTheme();
    ...
    ctx.fillStyle = theme.accent;
    roundRect(ctx, pad, fy + 9, 4, 12, 2);
    ctx.fill();

    ctx.textAlign = 'left';
    ctx.fillStyle = theme.accent;
    ctx.font = 'bold 13px "Noto Sans TC", sans-serif';
    ctx.fillText('WS-Cards', pad + 10, textY);

    ctx.fillStyle = theme.textSubtle;
    ctx.font = '11px "Noto Sans TC", sans-serif';
    ctx.fillText('收藏市場戰報', pad + 82, textY);
...
*/
function _drawCardPlaceholderLight(ctx, x, y, w, h, theme) {
    theme = theme || getStatsExportTheme();
