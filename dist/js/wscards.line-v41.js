...
class ImageDetectionManager {
    ...
    initEventListeners() {
        ...
        if (detectUploadedBtn) {
            detectUploadedBtn.addEventListener('click', () => this.detectUploadedImage());
        }
    ...
    async processImage(blob) {
        try {
            // 使用 Tesseract.js 進行真實的 OCR 辨識
            await this.performOCR(blob);
        } catch (error) {
    ...
    /**
     * 使用 Tesseract.js 執行真實的 OCR 辨識
     */
    async performOCR(blob) {
        try {
