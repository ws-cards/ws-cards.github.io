                        ...
                        yAxes: [{
                            ...
                            ticks: {
                                ...
                                callback: function(value) {
                                    // 原本 1000 以上就換成 (value/1000).toFixed(0)+'k'，
                                    // 於是 900 / 1300 / 1400 全部印成「1k」，1500 印成「2k」——
                                    // 大多數威夏卡就落在這個區間，整條 Y 軸讀起來會是 1k,1k,1k,2k。
                                    // 只有到萬位才值得縮寫。
...
if (elChange) {
    if (changePercent === null) {
        elChange.textContent = '--';
        triggerAnimate(elChange, 'price-summary-value price-flat');
    } else {
...
/**
* 建立下載用組合圖（價格折線 + 庫存直條）並回傳 base64
* 時間範圍跟隨 priceTimeRange 選單
* @returns {string}
*/
function _createStatsComboChartBase64() {
    if (!_rawPriceLabels || _rawPriceLabels.length === 0) return '';
    ...
    var priceData = filtered.data;
    if (!labels || labels.length === 0) return '';

    var stockData = _alignStockToLabels(labels);
    var weekly = _aggregateWeeklyForStatsExport(labels, priceData, stockData, 90);
    labels = weekly.labels;
    priceData = weekly.priceData;
    stockData = weekly.stockData;
    var hasStock = stockData.some(function(v) { return v !== null && v !== undefined && !isNaN(Number(v)); });
    ...
    var yAxes = [{
        ...
        ticks: {
            ...
            fontSize: 11,
            callback: function(value) {
                // 同上：只有到萬位才縮寫，否則 900/1300/1400 全變成「1k」
                if (value >= 10000) return (value / 1000).toFixed(0) + 'k';
                return value.toLocaleString('en-US', { maximumFractionDigits: 0 });
            }
        }
    ...
    try {
                ...
                scales: {
                    xAxes: [{
                        ...
                        ticks: {
                            ...
                            maxTicksLimit: 7,
                            callback: function(value) {
                                return _formatStatsChartXTick(value);
                            }
                        }
                    }],
                    yAxes: yAxes
        ...
        var base64 = exportChart.toBase64Image();
        exportChart.destroy();
        return base64;
    } catch (e) {
        console.error('建立組合圖失敗:', e);
        if (exportChart) {
            try { exportChart.destroy(); } catch (destroyErr) { /* ignore */ }
        }
        return '';
    }
}
...
// 檢查是否有價格圖表
if (!myChart) {
    Swal.fire({ icon: 'warning', title: '還沒有可下載的圖表', text: '請先查詢一張卡號，載入價格走勢後再下載。', confirmButtonText: '知道了' });
    return;
}
...
// 取得組合圖 base64（價格折線 + 庫存直條），失敗時 fallback 至頁面價格圖
var chartBase64 = '';
try {
    chartBase64 = _createStatsComboChartBase64();
    if (!chartBase64 && myChart) {
        chartBase64 = myChart.toBase64Image();
    }
} catch (e) {
    console.error('取得圖表圖片失敗:', e);
    try {
        if (myChart) chartBase64 = myChart.toBase64Image();
    } catch (fallbackErr) {
        console.error('fallback 圖表圖片失敗:', fallbackErr);
    }
}
...
/**
* 合成統計圖片 Canvas 並觸發下載
* 布局參考：卡圖左側 + 右側資訊 / 橫排4格價格摘要 / 左統計右圖表 / 底部浮水印
...
function _composeStatsCanvas(info) {
    ...
    var C_CARD      = '#ffffff';  // 卡片白底
    var C_BORDER    = '#e8eaed';  // 邊框
    var C_TEXT1     = '#1a1a2e';  // 主文字（深藍黑）
    var C_TEXT2     = '#555770';  // 次要文字
    var C_TEXT3     = '#8a8fa8';  // 標籤文字
    var C_ACCENT    = '#667eea';  // 品牌主色（藍紫）
    var C_UP        = '#e53e3e';  // 漲（紅）
    var C_DOWN      = '#38a169'; // 跌（綠）
    var C_NEUTRAL   = '#718096'; // 持平（灰）
    ...
    function drawBadge(text, x, y, bgColor, textColor) {
        ...
        var bw = tw + 18;
        var bh = 26;
        ctx.fillStyle = bgColor || '#eef0ff';
        roundRect(ctx, x, y, bw, bh, 6);
        ctx.fill();
        ctx.fillStyle = textColor || C_ACCENT;
        ctx.textAlign = 'left';
        ctx.fillText(text, x + 9, y + 18);
        return bw + 8; // 回傳佔用寬度（含間隔）
    }
    ...
    var CI_BOX_X = hx + Math.floor((LEFT_COL_W - CI_BOX_W) / 2);
    var CI_BOX_Y = hy + Math.floor((hh - CI_BOX_H) / 2);
    var CI_W = CI_BOX_W, CI_H = CI_BOX_H;
    var CI_X = CI_BOX_X, CI_Y = CI_BOX_Y;

    if (info.cardImage) {
        try {
            var ci_srcW = info.cardImage.naturalWidth || info.cardImage.width || CI_BOX_W;
            var ci_srcH = info.cardImage.naturalHeight || info.cardImage.height || CI_BOX_H;
            var ci_scale = Math.min(CI_BOX_W / ci_srcW, CI_BOX_H / ci_srcH);
            CI_W = ci_srcW * ci_scale;
            CI_H = ci_srcH * ci_scale;
            CI_X = CI_BOX_X + (CI_BOX_W - CI_W) / 2;
            CI_Y = CI_BOX_Y + (CI_BOX_H - CI_H) / 2;
            ...
            ctx.save();
            ctx.beginPath();
            roundRectPath(ctx, CI_X, CI_Y, CI_W, CI_H, 8);
            ctx.clip();
            ctx.drawImage(info.cardImage, CI_X, CI_Y, CI_W, CI_H);
            ctx.restore();
            ...
            _drawCardPlaceholderLight(ctx, CI_BOX_X, CI_BOX_Y, CI_BOX_W, CI_BOX_H);
        }
    } else {
        _drawCardPlaceholderLight(ctx, CI_BOX_X, CI_BOX_Y, CI_BOX_W, CI_BOX_H);
    }
    ...
    var pcItems = [
        ...
        { label: '庫存數量', value: info.currentStock || '--', unit: '件',    change: info.dayChangeStock || '', changeLabel: '與昨日相比', color: '#5a67d8', arrow: '' },
    ...
    for (var pi = 0; pi < 4; pi++) {
        ...
        if (pci.change) {
            ...
            if (pci.changeLabel !== '') {
                // 價格日變 / 庫存日變：負 → 紅，正 → 綠
                if (pci.change.indexOf('-') >= 0) changeColor = '#e53e3e';
                else if (pci.change.indexOf('+') >= 0 || parseFloat(pci.change) > 0) changeColor = '#38a169';
            } else {
                // 日期格式 → 用卡片主色
    ...
    var badgeX = TX;
    if (info.cardRare) {
        badgeX += drawBadge(info.cardRare, badgeX, TY, '#eef0ff', C_ACCENT);
    }
    if (info.cardColor) {
        var colorBg = { '藍':'#ebf4ff', '赤':'#fff5f5', '紅':'#fff5f5', '黃':'#fffae6', '黄':'#fffae6', '綠':'#f0fff4', '緑':'#f0fff4' }[info.cardColor] || '#f7f7f7';
        var colorFg = { '藍':'#3182ce', '赤':'#e53e3e', '紅':'#e53e3e', '黃':'#d69e2e', '黄':'#d69e2e', '綠':'#38a169', '緑':'#38a169' }[info.cardColor] || '#555';
        badgeX += drawBadge(info.cardColor, badgeX, TY, colorBg, colorFg);
    }
    if (info.cardLevel) {
        badgeX += drawBadge('Lv.' + info.cardLevel, badgeX, TY, '#f0fff4', '#38a169');
    }
    if (info.cardPower) {
        badgeX += drawBadge('P.' + info.cardPower, badgeX, TY, '#fff5f5', '#e53e3e');
    }
    TY += 36;
    ...
    if (info.cardNo) {
        ...
        ctx.textAlign = 'left';
        ctx.fillText(info.cardNo, TX, TY);
        TY += 20;
    }
    ...
    var TABLE_W = TW;
    var TABLE_X = TX;
    var TABLE_H = tableRows.length * ROW_H;

    // Table 底色（white 背景，用 roundRectPath clip）
    ctx.fillStyle = '#ffffff';
    ctx.beginPath();
    roundRectPath(ctx, TABLE_X, TY, TABLE_W, TABLE_H, 6);
    ctx.closePath();
    ctx.fill();
    ...
    for (var ti = 0; ti < tableRows.length; ti++) {
        if (ti % 2 === 0) {
            ...
            ctx.fillRect(TABLE_X + 1, TY + ti * ROW_H, TABLE_W - 2, ROW_H);
        }
    }

    // 行分隔線 + 內容
    for (var tj = 0; tj < tableRows.length; tj++) {
        var tr = tableRows[tj];
        var ry = TY + tj * ROW_H;
        var textMidY = ry + ROW_H / 2 + 4;

        if (tj > 0) {
            ctx.strokeStyle = C_BORDER;
            ctx.lineWidth = 0.5;
            ctx.beginPath();
            ctx.moveTo(TABLE_X, ry);
            ctx.lineTo(TABLE_X + TABLE_W, ry);
            ctx.stroke();
        }
        ...
        ctx.font = '11px "Noto Sans TC", sans-serif';
        ctx.textAlign = 'left';
        ctx.fillText(tr.label, TABLE_X + 26, textMidY);

        // 值（右對齊）
        ctx.fillStyle = C_TEXT1;
        ctx.font = '600 11px "Noto Sans TC", sans-serif';
        ctx.textAlign = 'right';
        var labelW = ctx.measureText(tr.label).width;
        var valueMaxW = TABLE_W - 26 - labelW - 12;
        ctx.fillText(_truncateText(ctx, tr.value, valueMaxW < 60 ? 80 : valueMaxW), TABLE_X + TABLE_W - 8, textMidY);
    }

    // Table 外框（drawn last to cover row bg overflow at corners）
    ctx.strokeStyle = C_BORDER;
    ctx.lineWidth = 1;
    ctx.beginPath();
    roundRectPath(ctx, TABLE_X, TY, TABLE_W, TABLE_H, 6);
    ...
    // ════════════════════════════════
    // Zone 2: 圖表面板（標題列 + 左統計 + 右圖表）
    // ════════════════════════════════
    var cy0 = hy + hh + SECTION_GAP;
    var cpW = IMG_WIDTH - PAD * 2;
    var cpH = CHART_PANEL_H;
    ...
    ctx.fill();
    // 底部不圓角的補丁
    ctx.fillRect(PAD, cy0 + TITLE_BAR_H - 10, cpW, 10);
    ...
    roundRect(ctx, titleBadgeX, titleBadgeY, titleBadgeW, titleBadgeH, 10);
    ctx.fill();
    ...
    // 右側最新價格 badge
    // var latestPriceText = '最新  ' + (info.currentPrice || '--');
    // ctx.font = 'bold 14px "Noto Sans TC", sans-serif';
    // var lpW = ctx.measureText(latestPriceText).width + 24;
    // var lpX = PAD + cpW - lpW - 14;
    // ctx.fillStyle = '#eef0ff';
    // roundRect(ctx, lpX, cy0 + 13, lpW, 28, 6);
    // ctx.fill();
    ...
    if (info.chartBase64) {
        ...
        chartImg.onload = function() {
            ...
            _drawStatsFooter(ctx, IMG_WIDTH, IMG_HEIGHT, FOOTER_H, cy0 + cpH + SECTION_GAP, PAD, info);
...
* 繪製底部資訊列（浮水印 + 日期 + 來源）
*/
function _drawStatsFooter(ctx, imgW, imgH, footerH, fy, pad, info) {
    var C_TEXT3 = '#8a8fa8';
    var C_ACCENT = '#667eea';
    var C_BORDER = '#e8eaed';
    ...
    var rightText = '資料來源：遊遊亭　製表時間：' + dateStr;
...
/**
* 繪製卡圖佔位框（淺色版，用於新版白底下載圖）
*/
function _drawCardPlaceholderLight(ctx, x, y, w, h) {
    ...
    ctx.stroke();
    ctx.setLineDash([]);
    ctx.fillStyle = '#c4c9d8';
    ctx.font = '36px sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText('🃏', x + w / 2, y + h / 2 + 8);
    ctx.fillStyle = '#b0b5c4';
    ctx.font = '12px "Noto Sans TC", sans-serif';
    ctx.fillText('No Image', x + w / 2, y + h / 2 + 32);
}
...
ctx.font = '14px "Noto Sans TC", sans-serif';
ctx.textAlign = 'center';
ctx.fillText('WS-Cards (ws-cards.cloud)  ·  資料來源：遊遊亭  ·  ' + new Date().toLocaleDateString('zh-TW'), imgWidth / 2, footerY + 35);
}
...
try {
    ...
    // 原始錯誤留在 console 給開發者，讀者只需要知道能怎麼做
    Swal.fire({ icon: 'error', title: '圖片產生失敗', text: '請重新整理頁面後再試一次。', confirmButtonText: '知道了' });
}
}
 ...
 * 答案抵達 — 這一頁唯一被編排過的時刻
 * ==========================================================================
 * 查一張卡原本是一次跳接：三個轉圈圈停掉，然後圖片、圖表、四個數字同時彈出來。
 * 這個模組把它排成一個約 700ms 的動作，順序照讀者關心的順序：
 *
 *   t+0     卡面從剛剛按下的縮圖變形過來（View Transitions）
 *   t+120   售價從「七天前的基準」滾到今天的數字
 *   t+180   價格線由左往右畫出來（庫存線 t+300 跟上，同一個方向）
 *   t+520   近 30 日最高落點
 *   t+560   近 30 日最低落點
 *
 * 三個原則貫穿整段：
 *   1. 全部包在既有函式外面。這個模組整段拿掉，頁面回到原本的行為，不會壞。
 *   2. 每一段都從「已經看得見」的狀態出發 —— 動畫沒跑完（節流的分頁、
 *      動畫時間軸沒推進）也一定讀得到數字。價格不能有隱形的狀態。
 *   3. prefers-reduced-motion 一律直接跳到終點，不是放慢。
 * ========================================================================== */
(function () {
    'use strict';
    ...
    // 指數式煞尾：起步快、收尾慢，跟這一頁的 --ease-out 是同一條曲線
    function easeOut(t) {
     ...
     * 1. 數字滾動
     * ---------------------------------------------------------------------
     * 從 0 滾到終值是廣告的做法，對查價的人沒有意義。
     * 這裡的起點都是有話要說的：售價從「七天前的基準」滾上來，
     * 最高／最低從「今天的價格」往兩邊跑開 —— 於是那一格真的畫出了 30 日的區間。
     * CSS 已經給這些元素 tabular-nums，所以數字在滾動時版面不會抖。
     */
    function rollNumber(el, from, to, render, delay) {
        if (!el) return;

        // 換卡比動畫快是常態（連點兩個卡號、或資料分兩次回來）。
        // 每顆元素只允許一個進行中的滾動：新的一啟動，舊的立刻作廢，
        // 否則舊的 rAF 會在新值寫好之後把畫面蓋回舊的中間值 ——
        // 那會讓價格停在一個「從來不存在的數字」上。
        var token = (el.__wsRollToken || 0) + 1;
        el.__wsRollToken = token;
        function stale() { return el.__wsRollToken !== token; }

        if (reduceMotion() || from === to || !isFinite(from) || !isFinite(to)) {
        ...
        function finish() {
            ...
            if (!stale()) el.textContent = render(to);
        }

        function frame(ts) {
            if (stale()) { done = true; return; }
            if (started === null) started = ts;
            var t = Math.min(1, (ts - started) / WS_ROLL_MS);
        ...
        // 保險：時間軸沒推進（節流的分頁）時也要落到終值，絕不留在中間值
        setTimeout(finish, (delay || 0) + WS_ROLL_MS + 400);
    }
    ...
    function landTile(item, delay) {
        if (!item || reduceMotion()) return;
        setTimeout(function () {
            item.classList.remove('ws-landed');
            void item.offsetWidth;
            item.classList.add('ws-landed');
        }, delay);
    }
     ...
     * 2. 價格摘要：讓四個數字照順序抵達
     * ---------------------------------------------------------------------
     * 原函式照舊跑完（它負責所有計算、日期、漲跌方向、範例卡提示），
     * 我們只在它寫完之後把終值接手，改成滾進去。
     */
    var originalUpdatePriceSummary = window.updatePriceSummary;
    if (typeof originalUpdatePriceSummary === 'function') {
        window.updatePriceSummary = function (priceData) {
            originalUpdatePriceSummary.apply(this, arguments);
            ...
            var current = readNum(elCurrent);
            var high = readNum(elHigh);
            var low = readNum(elLow);
            var change = readNum(elChange);

            // 沒有價格就不要表演，'--' 保持原樣
            if (!isFinite(current)) return;

            // 七日基準價：由現價與漲跌幅反推，跟卡片上寫的 % 必然一致
            var basis = isFinite(change) && change !== 0
                ? current / (1 + change / 100)
                : current;

            rollNumber(elCurrent, basis, current, yen, 120);
            if (isFinite(change)) {
                rollNumber(elChange, 0, change, pct, 120);
            }
            // 最高／最低從今天的價格往兩邊跑開，把 30 日的區間畫出來
            if (isFinite(high)) rollNumber(elHigh, current, high, yen, 520);
            if (isFinite(low)) rollNumber(elLow, current, low, yen, 560);

            landTile(card.querySelector('.price-summary-item.current'), 120);
            landTile(card.querySelector('.price-summary-item.change'), 180);
            landTile(card.querySelector('.price-summary-item.highest'), 520);
            landTile(card.querySelector('.price-summary-item.lowest'), 560);
        };
    }
     ...
     * 3. 圖表：線由左往右畫出來
     * ---------------------------------------------------------------------
     * 只裁切資料集，不裁切格線與座標軸 —— 所以方格紙一開始就完整攤在那裡，
     * 線是落在既有的紙上，而不是整張圖一起淡入。
     * Chart.js 自己的補間關掉（duration 0），節奏統一由這裡出。
     */
    var WIPE_MS = 620;

    function runWipe(chart, delay) {
        if (!chart || typeof chart.draw !== 'function') return;

        // 同一張圖只允許一次進行中的揭露（快速連切時間範圍會疊起來），
        // 舊的一旦作廢就不能再畫，否則會把線又蓋回半截
        var token = (chart.$wsWipeToken || 0) + 1;
        chart.$wsWipeToken = token;
        function stale() { return chart.$wsWipeToken !== token; }

        function paint(p) {
            chart.$wsWipe = p;
            try { chart.draw(); } catch (e) { chart.$wsWipe = 1; }
        }
        ...
        function frame(ts) {
            if (stale()) { done = true; return; }
            if (started === null) started = ts;
            var t = Math.min(1, (ts - started) / WIPE_MS);
            paint(easeOut(t));
            if (t < 1) { raf = requestAnimationFrame(frame); } else { finish(); }
        }

        // 起點留 0.04 而不是 0：時間軸萬一沒推進，至少不是一張空圖
        paint(0.04);
        setTimeout(function () {
            if (stale()) return;
            raf = requestAnimationFrame(frame);
        }, delay || 0);
        // 保險：任何情況下都要落到完整的線，不能停在半截
        setTimeout(finish, (delay || 0) + WIPE_MS + 500);
    }
        ...
        Chart.plugins.register({
            ...
            afterInit: function (chart) {
                if (chart.options && chart.options.animation) {
                    // 換時間範圍時，Chart 預設會把每個點補間到新位置，
                    // 點數一變就是一團亂跑。關掉，改成整條線重畫一次。
                    chart.options.animation.duration = 0;
                }
                var id = chart.canvas && chart.canvas.id;
                // 同一個方向、往下錯開，讀起來是一支筆掃過兩張圖，不是兩件事
                var delay = id === 'myStockChart' ? 300 : 180;
                setTimeout(function () { runWipe(chart, delay); }, 0);
            },
            beforeDatasetsDraw: function (chart) {
                ...
                ctx.clip();
                chart.$wsClipped = true;
            },
            afterDatasetsDraw: function (chart) {
                if (chart.$wsClipped) {
                    chart.ctx.restore();
                    chart.$wsClipped = false;
                }
            }
        });
    }

    // 換時間範圍時重畫一次：讀者看到的是「新的區間被重新畫出來」
    ['applyPriceTimeFilter', 'applyStockTimeFilter'].forEach(function (name) {
        var original = window[name];
        if (typeof original !== 'function') return;
        window[name] = function () {
            var r = original.apply(this, arguments);
            runWipe(name === 'applyPriceTimeFilter' ? window.myChart : window.myStockChart, 0);
            return r;
        };
    });
     ...
     * 4. 卡面：從按下的縮圖變形過來
     * ---------------------------------------------------------------------
     * 進階選項的結果條裡是真的縮圖，圖檔已經在瀏覽器快取裡。
     * 所以按下去的瞬間就能把同一張圖指給卡面預覽，再用 View Transitions
     * 讓它從彈窗裡「飛」到框裡 —— 不是彈窗關掉、圖片另外淡入兩件事。
     * 沒有 View Transitions 或關掉動態效果時，退回原本的行為。
     */
    var VT_NAME = 'ws-card-face';

    function canMorph() {
        return typeof document.startViewTransition === 'function' && !reduceMotion();
    }

    var originalSelectAdvancedCard = window.selectAdvancedCard;
    if (typeof originalSelectAdvancedCard === 'function') {
        window.selectAdvancedCard = function (cardId) {
            var thumb = null;
            var strip = document.getElementById('advResultStrip');
            if (strip && cardId) {
                var btn = strip.querySelector('[data-card-id="' + String(cardId).replace(/"/g, '\\"') + '"]');
                if (btn) thumb = btn.querySelector('img');
            }

            if (!canMorph() || !thumb || !thumb.currentSrc) {
                return originalSelectAdvancedCard.apply(this, arguments);
            }

            var target = document.getElementById('cardImg');
            var placeholder = document.querySelector('.image-placeholder');
            var cachedSrc = thumb.currentSrc;
            var args = arguments;
            var self = this;
            ...
            var transition = document.startViewTransition(function () {
                ...
                if (target) {
                    ...
                    // 縮圖已經載過同一個檔，這裡是快取命中，不會出現空框
                    target.src = cachedSrc;
                    target.style.display = 'block';
                    target.style.opacity = '1';
                }
                if (placeholder) placeholder.style.display = 'none';
                originalSelectAdvancedCard.apply(self, args);
            });
    ...
    /* ---------------------------------------------------------------------
     * 5. 篩選階梯：沒輪到的那一階是鎖著的，不是空的
     * ---------------------------------------------------------------------
     * 原本 #cardNumber 由 JS 直接寫 inline visibility:hidden，
     * 於是「卡號」標籤底下留著一塊 47px 的空洞 —— 看起來像壞了。
     * 這裡不改那三個呼叫點，而是盯著它的 style 變化，把狀態鏡射到整組上，
     * 並補一行說明它在等什麼。
     */
    function initCascade() {
        var numberSel = document.getElementById('cardNumber');
        var titleSel = document.getElementById('cardTitle');
        var standardSel = document.getElementById('cardStandard');
        if (!numberSel || !titleSel || !standardSel) return;

        function groupOf(sel) {
            return sel ? sel.closest('.filter-group') : null;
        }

        function ensureNote(sel, text) {
            var group = groupOf(sel);
            if (!group) return;
            if (group.querySelector('.filter-waiting')) return;
            var note = document.createElement('div');
            note.className = 'filter-waiting';
            note.textContent = text;
            sel.parentNode.insertBefore(note, sel);
        }

        ensureNote(titleSel, '先選作品系列');
        ensureNote(numberSel, '先選產品');

        function setLocked(sel, locked) {
            var group = groupOf(sel);
            if (!group) return;
            var was = group.classList.contains('is-locked');
            group.classList.toggle('is-locked', locked);
            if (was && !locked) {
                group.classList.add('just-unlocked');
                setTimeout(function () { group.classList.remove('just-unlocked'); }, 500);
            }
        }

        var EMPTY = '000/000-000';

        function sync() {
            setLocked(titleSel, !standardSel.value || standardSel.value === EMPTY);
            // 卡號那一階沿用頁面自己的判斷（inline visibility），不另立一套規則
            setLocked(numberSel, numberSel.style.visibility === 'hidden');
        }

        new MutationObserver(sync).observe(numberSel, {
            attributes: true,
            attributeFilter: ['style']
        });
        standardSel.addEventListener('change', sync);
        titleSel.addEventListener('change', sync);
        sync();
    }
