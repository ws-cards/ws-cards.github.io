...
function _composeStatsCanvas(info) {
    ...
    // ════════════════════════════════
    // Zone 1: Header（40% 卡圖 | 60% 右欄：文字+table+2x2價格卡）
    // ════════════════════════════════
    ...
    var pcItems = [
        { label: '目前價格',  value: info.currentPrice,  unit: '¥ JPY', sub: '',                 color: C_ACCENT,  isChange: false },
        { label: '近7天漲跌', value: info.changePercent, unit: '',       sub: '',                 color: C_NEUTRAL, isChange: true },
        { label: '30日最高',  value: info.highPrice,     unit: '¥ JPY', sub: info.highDate || '', color: C_UP,      isChange: false },
        { label: '30日最低',  value: info.lowPrice,      unit: '¥ JPY', sub: info.lowDate  || '', color: C_DOWN,    isChange: false }
    ];
    if (info.changePercent && String(info.changePercent).indexOf('-') >= 0) {
        pcItems[1].color = C_DOWN;
    } else if (info.changePercent && info.changePercent !== '--' && info.changePercent !== '0.0%' && info.changePercent !== '0%') {
        pcItems[1].color = C_UP;
    }
    ...
    for (var pi = 0; pi < 4; pi++) {
        ...
        ctx.fillText(pci.label, pcx + PC_CARD_W / 2, pcy + 24);

        // Value
        var pvStr = pci.value || '--';
        var pvColor = C_TEXT1;
        if (pci.isChange) {
            if (pvStr.indexOf('-') >= 0) pvColor = C_DOWN;
            else if (pvStr !== '--' && pvStr !== '0.0%' && pvStr !== '0%') pvColor = C_UP;
            else pvColor = C_NEUTRAL;
        }
        ctx.fillStyle = pvColor;
        ctx.font = 'bold 17px "Noto Sans TC", sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText(_truncateText(ctx, pvStr, PC_CARD_W - 8), pcx + PC_CARD_W / 2, pcy + 60);
        ...
        if (pci.sub) {
            ...
            ctx.textAlign = 'center';
            ctx.fillText(pci.sub, pcx + PC_CARD_W / 2, pcy + 78);
        }
        if (pci.unit) {
            ctx.fillStyle = C_TEXT3;
            ctx.font = '10px "Noto Sans TC", sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText(pci.unit, pcx + PC_CARD_W / 2, pcy + (pci.sub ? 92 : 78));
        }
    }
    ctx.textAlign = 'left';

    // ── 右欄文字資訊區 ──
    var TW = RC_W - PC_CARD_COL_W - 16;
    var TX = RC_X;
    var TY = hy + 16;
    ...
    TY += 8;
    var tableRows = [
        { iconColor: '#667eea', label: '收錄補充包', value: info.cardPack || '--' },
        { iconColor: '#48bb78', label: '首次發售',   value: info.cardFirstRelease || '--' },
        { iconColor: '#ed8936', label: '卡片種類',   value: info.cardKind || '--' },
        { iconColor: '#ed64a6', label: '特徵',       value: info.cardFeatures || '--' }
    ];
    var ROW_H = 28;
    var TABLE_W = TW;
    var TABLE_X = TX;
    ...
    for (var tj = 0; tj < tableRows.length; tj++) {
        ...
        ctx.beginPath();
        ctx.arc(TABLE_X + 12, ry + ROW_H / 2, 8, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = '#ffffff';
        ctx.font = 'bold 9px sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText(String(tj + 1), TABLE_X + 12, ry + ROW_H / 2 + 3.5);
