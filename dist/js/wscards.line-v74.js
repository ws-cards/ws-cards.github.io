...
function _composeStatsCanvas(info) {
    ...
    for (var i = 0; i < summaryItems.length; i++) {
        ...
        ctx.fillText(si.label, sx + sCardW / 2, sy0 + 23);
        ...
        ctx.textAlign = 'center';
        ctx.fillText(vStr, sx + sCardW / 2, sy0 + 56);

        // 下方小單位文字
        ctx.fillStyle = C_TEXT3;
        ctx.font = '11px "Noto Sans TC", sans-serif';
        ctx.fillText(si.isChange ? '' : '¥ JPY', sx + sCardW / 2, sy0 + 77);

        // 箭頭（漲跌）
        if (si.arrow) {
            ctx.fillStyle = si.arrowColor;
            ctx.font = 'bold 13px sans-serif';
            ctx.fillText(si.arrow, sx + 14, sy0 + 56);
        }
    }
