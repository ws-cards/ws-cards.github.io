...
var _typeaheadMatches = [];
var _cardNameMatches = [];
var _typeaheadVisibleCount = 20;
...
function loadTypeaheadSuggestions() {
    fetch(_typeaheadSourceUrl)
        ...
        .then(function(items) {
            if (!Array.isArray(items)) throw new Error('作品建議資料格式不正確');
            var ta = $input.data('typeahead');
            if (!ta) return;
            ta.source = items;
            ta.options.source = items;
            if ($input.val().trim()) ta.lookup();
        })
        .catch(function(error) {
            console.error('作品建議清單載入失敗:', error);
        });
}
loadTypeaheadSuggestions();
// ====================================================
...
(function() {
    ...
    ta.render = function(items) {
        _typeaheadMatches = Array.isArray(items) ? items : [];
...
function searchByCardName(keyword) {
    ...
    fetchCardDictionary(function() {
        ...
        _cardNameMatches = _cardDictionary.filter(function(item) {
