...
function _composeStatsCanvas(info) {
    ...
    // ════════════════════════════════
    // Zone 1: Header（卡圖 + 卡片資訊）
    // ════════════════════════════════
    var hx = PAD, hy = PAD, hw = IMG_WIDTH - PAD * 2, hh = HEADER_H;
    drawCard(hx, hy, hw, hh);

    // 卡片圖片（左側）— 以配置框為界，維持原始比例(contain)，避免橫向圖被壓縮成直的
    var CI_BOX_W = 160, CI_BOX_H = 218;
    var CI_BOX_X = hx + 20, CI_BOX_Y = hy + (hh - CI_BOX_H) / 2;
    // 預設與配置框相同（placeholder 使用）
    var CI_W = CI_BOX_W, CI_H = CI_BOX_H;
    var CI_X = CI_BOX_X, CI_Y = CI_BOX_Y;

    if (info.cardImage) {
        try {
            // 依原圖實際比例縮放（contain），於配置框內置中
            ...
            _drawCardPlaceholderLight(ctx, CI_X, CI_Y, CI_W, CI_H);
        }
    } else {
        _drawCardPlaceholderLight(ctx, CI_X, CI_Y, CI_W, CI_H);
    }

    // 右側文字資訊（以配置框邊界為基準，避免橫向圖縮放後文字位移）
    var TX = CI_BOX_X + CI_BOX_W + 22;
    var TY = hy + 28;
    var TW = hw - (TX - hx) - 20;
    ...
    if (info.cardName) {
        ...
        ctx.textAlign = 'left';
        var maxCN = TW;
        ctx.fillText(_truncateText(ctx, info.cardName, maxCN), TX, TY + 26);
        TY += 42;
    }

    // Rare / Color / Level 標籤列
    var badgeX = TX;
    if (info.cardRare) {
    ...
    if (info.productName) {
        ...
        ctx.fillText(_truncateText(ctx, info.productName, TW), TX, TY);
        TY += 22;
    }
    ...
    if (info.seriesName) {
        ...
        ctx.fillText(_truncateText(ctx, info.seriesName, TW), TX, TY);
    }

    // ════════════════════════════════
    // Zone 2: 價格摘要（橫排 4 格）
    // ════════════════════════════════
    var sy0 = hy + hh + SECTION_GAP;
    var summaryItems = [
        { label: '目前價格', value: info.currentPrice },
        { label: '當月最高', value: info.highPrice,   arrow: '↑', arrowColor: C_UP },
        { label: '當月最低', value: info.lowPrice,    arrow: '↓', arrowColor: C_DOWN },
        { label: '近7天漲跌', value: info.changePercent, isChange: true }
    ];
    var sCardW = (IMG_WIDTH - PAD * 2 - 10 * 3) / 4;
    var sCardH = SUMMARY_H;

    for (var i = 0; i < summaryItems.length; i++) {
        var si = summaryItems[i];
        var sx = PAD + i * (sCardW + 10);
        drawCard(sx, sy0, sCardW, sCardH);

        // Label
        ctx.fillStyle = C_TEXT3;
        ctx.font = '13px "Noto Sans TC", sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText(si.label, sx + sCardW / 2, sy0 + 28);

        // Value（大字）
        var vStr = si.value || '--';
        var vColor = C_TEXT1;
        if (si.isChange) {
            if (vStr.indexOf('-') >= 0) vColor = C_DOWN;
            else if (vStr !== '--' && vStr !== '0.0%' && vStr !== '0%') vColor = C_UP;
            else vColor = C_NEUTRAL;
        }
        ctx.fillStyle = vColor;
        ctx.font = 'bold 26px "Noto Sans TC", sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText(vStr, sx + sCardW / 2, sy0 + 72);

        // 下方小單位文字
        ctx.fillStyle = C_TEXT3;
        ctx.font = '12px "Noto Sans TC", sans-serif';
        ctx.fillText(si.isChange ? '' : '¥ JPY', sx + sCardW / 2, sy0 + 95);

        // 箭頭（漲跌）
        if (si.arrow) {
            ctx.fillStyle = si.arrowColor;
            ctx.font = 'bold 14px sans-serif';
            ctx.fillText(si.arrow, sx + 14, sy0 + 72);
        }
    }

    // ════════════════════════════════
    // Zone 3: 圖表面板（標題列 + 左統計 + 右圖表）
    // ════════════════════════════════
    var cy0 = sy0 + SUMMARY_H + SECTION_GAP;
    ...
    ctx.textAlign = 'left';
    var titleText = (info.seriesName ? _truncateText(ctx, info.seriesName, 300) : 'WS-Cards') + '  價格 / 庫存走勢';
    ctx.fillText(titleText, PAD + 18, cy0 + 33);
