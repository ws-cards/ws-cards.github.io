...
function _setSearchActionButtonState(state) {
    ...
    button.title = nextState === 'search' ? '搜尋' : '複製';
    button.setAttribute('aria-label', nextState === 'search' ? '搜尋' : '複製');
...
function _periodText(months) {
    if (months === 1) return '(一月內)';
    if (months === 3) return '(三月內)';
...
function applyAdvancedFilters() {
    ...
    if (!filtered.length) {
        strip.innerHTML = '<div class="adv-result-empty">沒有符合條件的卡片</div>';
        return;
    }
    ...
    // 更新「下載清單」按鈕的 disabled 狀態
    var downloadBtn = document.getElementById('advDownloadListBtn');
    if (downloadBtn) {
...
function openAdvancedFilterModal() {
    ...
    loadProductContentCards(titleCode)
        ...
        .catch(function(err) {
            ...
            clearAdvancedProductCache();
            if (strip) strip.innerHTML = '<div class="adv-result-empty">無法載入此產品的卡片資料</div>';
 ...
 */
function validateAdvancedFiltersForExport() {
    var strip = document.getElementById('advResultStrip');
    if (!strip) return false;
    
    var items = strip.querySelectorAll('.adv-result-item');
    if (items.length === 0) {
        if (typeof Swal !== 'undefined') {
            Swal.fire({
                icon: 'warning',
 ...
 */
function getDisplayedAdvancedCards() {
    var cards = [];
    var strip = document.getElementById('advResultStrip');
    if (!strip) return cards;
    
    var items = strip.querySelectorAll('.adv-result-item');
    var sourceCards = (_advProductCache && _advProductCache.cards) ? _advProductCache.cards : [];
    ...
    items.forEach(function(btn) {
        ...
        // 找到對應的卡片數據
        var cardData = sourceCards.find(function(c) {
            return (c.id || c.cardno) === cardId;
        });
        
        if (cardData) {
            cards.push({
                id: cardId,
                name: cardData.name || cardData.cardname || cardId,
                rarity: cardData.rarity || cardData.cardrare || '-',
                price: '--',
                stock: '--'
            });
        }
...
/**
 * 使用 Canvas 繪製卡片清單
 * @param {Array} cards - 卡片數據陣列
 * @returns {Promise<Canvas>} - Canvas 對象
 */
function drawAdvancedCardListImage(cards) {
    return new Promise(function(resolve, reject) {
        ...
        // 計算畫布尺寸
        var cols = Math.min(COLS, cards.length);
        var rows = Math.ceil(cards.length / COLS);
        var canvasWidth = cols * CARD_WIDTH + (cols + 1) * GAP + 2 * PADDING;
        var canvasHeight = rows * CARD_HEIGHT + (rows + 1) * GAP + 2 * PADDING;
        ...
        function loadAllCardImages() {
            cards.forEach(function(card, index) {
                var urls = buildCardImageUrls(card.id);
                var img = new Image();
                ...
                img.onerror = function() {
                    ...
                    fallback.src = './dist/img/cardback.png';
                };
        ...
        function drawCards() {
            cards.forEach(function(card, index) {
                ...
                var x = PADDING + col * (CARD_WIDTH + GAP);
                var y = PADDING + row * (CARD_HEIGHT + GAP);
                ...
                if (img) {
                    var imgHeight = (img.height / img.width) * CARD_WIDTH;
                    var maxImgHeight = CARD_HEIGHT - 100; // 預留底部空間
                    if (imgHeight > maxImgHeight) {
                        imgHeight = maxImgHeight;
                    }
                    ctx.drawImage(img, x, y, CARD_WIDTH, imgHeight);
                }
                
                // 繪製卡號區域（黑底）
                var infoY = y + CARD_HEIGHT - 95;
                ctx.fillStyle = '#000000';
                ctx.fillRect(x, infoY, CARD_WIDTH, 20);
                ctx.fillStyle = '#ffffff';
                ctx.font = 'bold 11px Arial';
                ctx.textAlign = 'center';
                ctx.fillText(card.id, x + CARD_WIDTH / 2, infoY + 15);
                ...
                ctx.textAlign = 'center';
                var displayName = card.name.substring(0, 20);
                ctx.fillText(displayName, x + CARD_WIDTH / 2, nameY);
                ...
                ctx.textAlign = 'center';
                ctx.fillText('¥' + (card.price || '--'), x + CARD_WIDTH / 2, priceY);
                
                // 繪製在庫
                var stockY = priceY + 18;
                ctx.fillStyle = '#999999';
                ctx.font = '11px Arial';
                ctx.textAlign = 'center';
                ctx.fillText('在庫: ' + (card.stock || '--'), x + CARD_WIDTH / 2, stockY);
            });
 ...
 * 下載進階篩選卡片清單為 PNG
 */
function downloadAdvancedCardList() {
    if (!validateAdvancedFiltersForExport()) {
        return;
    }
    
    // 提取卡片數據
    var cards = getDisplayedAdvancedCards();
    if (cards.length === 0) {
        if (typeof Swal !== 'undefined') {
            Swal.fire({
                icon: 'error',
    ...
    // 生成 Canvas
    drawAdvancedCardListImage(cards).then(function(canvas) {
        // 轉換為 Blob 並下載
        canvas.toBlob(function(blob) {
            var productName = '';
            var nameEl = document.getElementById('advProductName');
            if (nameEl) {
                productName = nameEl.textContent.replace(/\s+/g, '-');
            }
            
            var timestamp = new Date().getTime();
            var filename = 'WS-Cards-List-' + (productName || 'Export') + '-' + timestamp + '.png';
