...
function isCardNumberFormat(input) {
  ...
  var cardNumberPattern = /^[A-Z0-9]+(?:\/[A-Z0-9]+)?-[A-Z0-9]+[A-Z]*$/i;
...
async function setSelectorsFromCardParts(cardParts) {
  try {
    ...
    showSearchNotification('設置選擇器時發生錯誤: ' + error.message, 'error');
			...
			function changeTitleAdvanced(){	
			  console.log("changeTitleAdvanced");
     ...
     // }
      ...
      for (var i = 0; i < cardTitleSelect.options.length; i++) {
        ...
        if (value && value.toLowerCase().includes("/"+searchTarget)) {
          console.log('找到匹配的主題:', option.text, 'value:', value);
          option.selected = true;
          var haveFindNumber = false;

          // 觸發變更事件
          changeTitleAdvanced();
          var cardNumberSelect = document.getElementById('cardNumber');
		  console.log('length:'+cardNumberSelect.options.length);
		  // 遍歷所有選項找到匹配的
     		 for (var j = 0; j < cardNumberSelect.options.length; j++) {
		        var optionNumber = cardNumberSelect.options[j];
				var valueNumber = optionNumber.value;
				var searchsSuffixTarget = suffix.toLowerCase();		
				console.log('findNumber searchTarget:'+searchsSuffixTarget+' value:'+valueNumber);
				if (valueNumber && valueNumber.toLowerCase().includes(searchsSuffixTarget)) {
					console.log('找到匹配的卡號:', optionNumber.text, 'value:', valueNumber);
